...
import { useVolunteerAssessments, type NewAssessmentPayload } from '@/features/volunteers/composables/useVolunteerAssessments'
...
const { rows: assessments, loading: assessmentsLoading, submit: submitAssessment } = useVolunteerAssessments(userId)
...
const tabItems = computed(() => {
  ...
  if (store.hasPerm('users.permissions')) items.push({ label: 'الصلاحيات', value: 'perms', slot: 'perms' })
...
<template>
  <div class="max-w-3xl mx-auto space-y-4">
    <UButton to="/volunteers" variant="ghost" color="neutral" icon="i-lucide-arrow-right" label="رجوع لقائمة المتطوعين" />
    ...
    <template v-else-if="volunteer">
        ...
        <div class="flex items-center gap-4">
          ...
          <div class="min-w-0 flex-1">
            ...
            <div class="flex flex-wrap gap-2 mt-2">
              ...
              <UBadge v-if="volunteer.team" color="neutral" variant="outline">{{ volunteer.team }}</UBadge>
            </div>
          </div>
        </div>
        ...
        <template v-if="canManage">
          ...
          <div class="flex flex-wrap items-end gap-3">
            <UFormField label="حالة التطوع">
              <USelectMenu v-model="editState" :items="stateOptions" value-key="value" label-key="label" class="w-40" />
            </UFormField>
          ...
          <UAlert v-if="adminFieldsError" color="error" variant="subtle" :title="adminFieldsError" icon="i-lucide-alert-circle" class="mt-3" />
        </template>
      </UCard>
      ...
      <UTabs :items="tabItems" default-value="data" class="w-full">
        <template #data>
          <UCard>
            <div class="grid sm:grid-cols-2 gap-4 p-2">
              <div><p class="text-xs text-dimmed">المنطقة</p><p>{{ volunteer.area || '—' }}</p></div>
              <div><p class="text-xs text-dimmed">العمر</p><p>{{ volunteer.age ?? '—' }}</p></div>
              <div><p class="text-xs text-dimmed">المؤهل الدراسي</p><p>{{ extra?.education || '—' }}</p></div>
              <div><p class="text-xs text-dimmed">الوظيفة</p><p>{{ extra?.job || '—' }}</p></div>
          ...
          <div class="space-y-4">
            ...
            <template v-else>
              ...
              <UCard v-for="a in assessments" :key="a.assessment_id">
                ...
                <p v-if="a.strengths" class="text-sm mt-2"><span class="text-dimmed">نقاط القوة:</span> {{ a.strengths }}</p>
                <p v-if="a.development" class="text-sm mt-1"><span class="text-dimmed">مجالات التطوير:</span> {{ a.development }}</p>
                <p class="text-sm mt-1"><span class="text-dimmed">جاهز لدور أكبر:</span> {{ formatBool(a.ready_for_more) }}</p>
                <p v-if="a.next_step" class="text-sm mt-1"><span class="text-dimmed">الخطوة القادمة:</span> {{ a.next_step }}</p>
              </UCard>
            </template>
            ...
            <UCard>
              ...
              <UAlert v-if="assessmentError" color="error" variant="subtle" :title="assessmentError" icon="i-lucide-alert-circle" class="mt-3" />
              <UButton label="حفظ التقييم" :loading="savingAssessment" class="mt-4" @click="onSubmitAssessment" />
            </UCard>
          </div>
        </template>
          ...
          <div class="space-y-4">
            ...
            <template v-else>
              ...
              <UCard v-for="n in notes" :key="n.note_id">
                ...
                <p class="text-xs text-dimmed mt-2">{{ n.author_name || 'غير معروف' }} · {{ formatDateDMY(n.created_at) }}</p>
              </UCard>
            </template>

            <UCard>
              <UTextarea v-model="noteBody" class="w-full" :rows="3" placeholder="ملاحظة جديدة..." />
              <UAlert v-if="noteError" color="error" variant="subtle" :title="noteError" icon="i-lucide-alert-circle" class="mt-3" />
              <UButton label="إضافة ملاحظة" :loading="savingNote" class="mt-3" @click="onSubmitNote" />
            </UCard>
          </div>
        </template>
        ...
        <template #activity>
          <UCard>
            <p class="text-dimmed text-sm text-center py-8">الميزة دي لسه مش متاحة</p>
          </UCard>
        </template>
      </UTabs>
    </template>
